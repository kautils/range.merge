...
struct merge{
    ...
    //@note not used
    bool is_contained(offset_type pos , int direction){
        return bool(
            !direction
    ...
    int is_same_section(typename bt_type::btree_search_result const& btres0,typename  bt_type::btree_search_result const& btres1){
    ...
    offset_type adjust_np_ovf(typename bt_type::btree_search_result const& i0,offset_type pos_min,offset_type  pos_max){
    ...
    bool is_adjust_diff(typename bt_type::btree_search_result const& i0,value_type diff,value_type input){
        auto i0_diff=
    ...
    void __a(value_type from,value_type to){
...
//    }
    ...
    //m.__a(0,20);
    m.set_diff(diff);
    m.__a(from,to); // 0,20 => cont-overflow expect [0,8] (0,30) 
    fclose(f);
